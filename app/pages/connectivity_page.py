...
from app.connectivity.runtime import create_health_transport, create_pairing_service
...
class ConnectivityPage(QWidget):
    def __init__(self, main_window=None, pairing_service: PairingService | None = None):
        ...
        self.service = pairing_service or create_pairing_service()
        self.transport = None
    ...
    def _build_interface(self):
        ...
        receive_button = QPushButton("Receive Signed Request")
        receive_button.clicked.connect(self.receive_request)
        ...
        receive_layout.addWidget(receive_button, 0, Qt.AlignRight)
    ...
    def toggle_listener(self):
        ...
        if allow_lan:
            answer = QMessageBox.question(
                ...
                "Allow trusted devices on your private network to contact ARC3 on TCP port 8766?",
        ...
        try:
            self.transport = create_health_transport(
                ...
                host="172.20.20.20" if allow_lan else "127.0.0.1",
                tls_certfile=(Path(tempfile.gettempdir()) / "arc3-conduit-tls" / "server.crt") if allow_lan else None,
                tls_keyfile=(Path(tempfile.gettempdir()) / "arc3-conduit-tls" / "server.key") if allow_lan else None,
            )
